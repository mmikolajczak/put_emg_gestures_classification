...
def load_full_dataset(dataset_dir_path: str, create_val_subset: bool = True, val_size: float = 0.15,
    ...
    X_train = np.load(osp.join(dataset_dir_path, 'X_train.npz'))['arr'].astype(np.float32)
    y_train = np.load(osp.join(dataset_dir_path, 'y_train.npz'))['arr'].astype(np.float32)
    X_test = np.load(osp.join(dataset_dir_path, 'X_test.npz'))['arr'].astype(np.float32)
    y_test = np.load(osp.join(dataset_dir_path, 'y_test.npz'))['arr'].astype(np.float32)
